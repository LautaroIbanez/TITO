...
describe('projectFixedIncome', () => {
  it('should project value correctly with a fixed term deposit', () => {
    ...
    // Check projection for 365 days
    expect(projection.length).toBe(366); 
    
    // Check final projected value
    const dailyInterest = (10000 * 0.10) / 365;
    const expectedFinalValue = initialValue + (dailyInterest * 365);
  ...
  it('should handle empty positions', () => {
    ...
    const positions: PortfolioPosition[] = [];
    const goals: InvestmentGoal[] = [
      { id: 'g1', name: 'Goal 1', targetAmount: 1200, targetDate: dayjs().add(1, 'year').format('YYYY-MM-DD'), initialDeposit: 1000, monthlyContribution: 0 },
    ];
