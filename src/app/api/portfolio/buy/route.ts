  ...
  try {
    ...
    switch (assetType) {
      case 'Stock': {
        ...
        const tx: PortfolioTransaction = { id: Date.now().toString(), date: new Date().toISOString(), type: 'Buy', assetType: 'Stock', symbol, quantity, price };
      ...
      case 'Bond': {
        ...
        totalCost = quantity * price; // Assuming price is per unit
        if (user.availableCash < totalCost) return NextResponse.json({ error: 'Insufficient funds' }, { status: 400 });
        ...
        const tx: PortfolioTransaction = { id: Date.now().toString(), date: new Date().toISOString(), type: 'Buy', assetType: 'Bond', ticker, quantity, price };
        user.transactions.push(tx);
        break;
      }
